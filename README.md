...
### Pairing With Social Automation Plugins

quality-guard protects shell commands and tool-output quality. It can run beside domain plugins that expose structured actions instead of shell scripts.

For X/Twitter automation, [TweetClaw](https://github.com/Xquik-dev/tweetclaw) adds an OpenClaw `tweetclaw` tool for scrape tweets, search tweets, search tweet replies, post tweets, post tweet replies, follower export, user lookup, media workflows, direct messages, monitor tweets, webhooks, and giveaway draws.

Use quality-guard to catch destructive shell commands while TweetClaw keeps X/Twitter API calls inside a catalog-backed OpenClaw tool. Keep TweetClaw's own approval prompts for visible actions, paid actions, monitors, webhooks, and DMs; quality-guard does not replace app-specific approval or spending review.
...
quality-guard 负责保护 shell 命令和工具输出质量。它也可以和使用结构化工具调用的领域插件一起运行。

如果需要 X/Twitter 自动化，[TweetClaw](https://github.com/Xquik-dev/tweetclaw) 提供 OpenClaw `tweetclaw` 工具，用于抓取推文、搜索推文、搜索推文回复、发布推文、发布回复、导出粉丝、查询用户、媒体流程、私信、推文监控、webhooks 和抽奖。

用 quality-guard 拦截破坏性 shell 命令，用 TweetClaw 将 X/Twitter API 调用限制在目录化 OpenClaw 工具内。对公开可见操作、付费操作、监控、webhooks 和私信，仍然保留 TweetClaw 自带的审批提示；quality-guard 不替代具体应用的审批或费用复核。
